...
class MakeNewOrder
{
    ...
    public function handle(AddToCart $event)
    {
        ...
        $brand = Brand::with('offers')->findorfail($product->brand_id);
        ...
        if($brand->offers->count() > 0){
            ...
            if(count($offer_price) > 0){
                $max_offer = max($offer_price);
                $key = array_search($max_offer, $offer_price);//get the key of the selected offer and macth to offer name
                $brand_offer_price = $price->price - $max_offer;
                $brand_offer_name = $offer_name[$key];
            }else{
                $brand_offer_price = $price->price;
                $brand_offer_name = "no offer";
            }
        }else{
            $brand_offer_price = $price->price;
            $brand_offer_name = "no offer";
        ...
        //compare category offer, brand offer, product offer and select one
        if($product_offer_price <= $category_offer_price && $product_offer_price <= $brand_offer_price)
        {
            $offered_price = $product_offer_price;
            $offered_name = $product_offer_name;
        }
        elseif($category_offer_price <= $brand_offer_price)
        {
            $offered_price = $category_offer_price;
            $offered_name = $category_offer_name;   
        }else{
            $offered_price = $brand_offer_price;
            $offered_name = $brand_offer_name;   
        }
